...
CREATE TABLE `ip_investment_pool` (
    ...
    `pool_type`             VARCHAR(64)  DEFAULT NULL            COMMENT '投资池类型（业务域，可扩展）：credit_bond=信用债 / offshore_bond=境外债 / convertible_bond=转债 / bond_product=债券产品 / special_account=专户 / stock=股票 / stock_product=股票产品 / fund=基金 / forbidden=禁止库 / observe=观察池 / blacklist=黑名单 / restricted=限制名单 / whitelist=白名单 / crmw=CRMW库 / other=其他；硬逻辑：credit_bond/crmw/forbidden/blacklist/observe/restricted',
    `pool_level`            INT          DEFAULT NULL            COMMENT '投资池层级：1=一级目录 / 2=子级池',
    `market_codes`          JSON         DEFAULT NULL            COMMENT '投资市场编码 JSON：SSE=上海证券交易所 / SZSE=深圳证券交易所 / CIBM=银行间市场 / BSE=北京证券交易所 / COMPANY=主体 / OTC=场外市场 / QDII=其他QDII市场 / JWCW=JWCW市场 / UNKNOWN=未知 / OTHER=其他',
    `variety_codes`         JSON         DEFAULT NULL            COMMENT '投资品种编码 JSON 数组，元素取值 dict_security_type.category_type：bond=债券 / stock=股票 / fund=基金 / company=公司主体 / index=指数 / warrant=权证 / trust=信托 / private_wealth=私募理财 / unknown=未知；示例：["bond"]、["bond","stock"]',
...
CREATE TABLE `ip_pool_auto_rule` (
    ...
    `task_code`  VARCHAR(50)  DEFAULT NULL            COMMENT '定时任务编码，对应 sys_scheduled_task.task_code，最长50字符',
