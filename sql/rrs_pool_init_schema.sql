...
CREATE TABLE `ip_investment_pool` (
    ...
    `pool_type`             VARCHAR(64)  DEFAULT NULL            COMMENT '投资池类型（业务域，可扩展）：credit_bond=信用债 / offshore_bond=境外债 / convertible_bond=转债 / bond_product=债券产品 / special_account=专户 / stock=股票 / stock_product=股票产品 / fund=基金 / forbidden=禁止库 / observe=观察池 / blacklist=黑名单 / restricted=限制名单 / whitelist=白名单 / crmw=CRMW库 / other=其他；硬逻辑：credit_bond/crmw/forbidden/blacklist/observe',
