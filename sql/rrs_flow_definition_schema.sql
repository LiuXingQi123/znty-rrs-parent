...
CREATE TABLE `wf_flow_definition` (
    ...
    `description` VARCHAR(1000) DEFAULT NULL             COMMENT '流程描述，简述业务场景及用途',
    `remark`      VARCHAR(1000) DEFAULT NULL             COMMENT '内部备注，不对发起人展示',
...
CREATE TABLE `wf_flow_version` (
    ...
    `publish_note`   VARCHAR(1000) DEFAULT NULL             COMMENT '发布备注，描述本次改动内容',
...
CREATE TABLE `wf_node_auto_config` (
    ...
    `auto_remark` VARCHAR(1000) DEFAULT NULL             COMMENT '节点备注（节点级属性，同一节点下各行冗余存储）',
    `crte_time`   DATETIME     DEFAULT NULL             COMMENT '创建时间',
    `updt_time`   DATETIME     DEFAULT NULL             COMMENT '修改时间',
    PRIMARY KEY (`id`)
) ENGINE = InnoDB
...
CREATE TABLE `wf_node_notify_config` (
    ...
    `notify_remark`   VARCHAR(1000)  DEFAULT NULL             COMMENT '通知节点备注说明',
    `crte_time`       DATETIME      DEFAULT NULL             COMMENT '创建时间',
    `updt_time`       DATETIME      DEFAULT NULL             COMMENT '修改时间',
    PRIMARY KEY (`id`)
) ENGINE = InnoDB
...
CREATE TABLE `wf_node_condition_config` (
    `id`               BIGINT       NOT NULL AUTO_INCREMENT  COMMENT '主键 ID',
    `node_id`          BIGINT       DEFAULT NULL             COMMENT '关联节点 ID（wf_flow_node.id）',
    `condition_remark` VARCHAR(1000) DEFAULT NULL             COMMENT '条件节点备注说明（具体分支逻辑在出线上配置）',
    `crte_time`        DATETIME     DEFAULT NULL             COMMENT '创建时间',
    `updt_time`        DATETIME     DEFAULT NULL             COMMENT '修改时间',
    PRIMARY KEY (`id`)
) ENGINE = InnoDB
...
CREATE TABLE `wf_node_condition_config_evt` (
    ...
    `id`               BIGINT       DEFAULT NULL             COMMENT '主键 ID',
    `node_id`          BIGINT       DEFAULT NULL             COMMENT '关联节点 ID（wf_flow_node.id）',
    `condition_remark` VARCHAR(1000) DEFAULT NULL             COMMENT '条件节点备注说明',
    `crte_time`        DATETIME     DEFAULT NULL             COMMENT '创建时间',
    `updt_time`        DATETIME     DEFAULT NULL             COMMENT '修改时间',
    -- 审计字段
    `opter_id`         VARCHAR(20)  DEFAULT NULL             COMMENT '经办人 ID',
...
CREATE TABLE `wf_flow_edge` (
    ...
    `route_action` VARCHAR(16)  DEFAULT NULL             COMMENT '流转动作：approve=通过 / reject=驳回 / submit=提交 / resubmit=重新提交 / auto=自动',
    `cond_logic`   VARCHAR(8)   DEFAULT NULL             COMMENT '多条件间逻辑关系：AND=且 / OR=或',
    `remark`       VARCHAR(1000) DEFAULT NULL             COMMENT '连线备注说明',
    `crte_time`    DATETIME     DEFAULT NULL             COMMENT '创建时间',
    `updt_time`    DATETIME     DEFAULT NULL             COMMENT '修改时间',
    PRIMARY KEY (`id`)
) ENGINE = InnoDB
