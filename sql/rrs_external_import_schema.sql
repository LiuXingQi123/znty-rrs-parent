-- ============================================================
-- znty-rrs 外部导入表 - 建库建表脚本
-- MySQL version: 8.0.33
-- 说明：
--   1. 存放由外部系统/同步导入的主数据表结构（非本系统业务运行态表）
--   2. 当前包含：rrs_securityinfo（证券主数据）；后续同类表在本文件追加
--   3. 与 rrs_security_pool_adjust_schema.sql 等业务脚本解耦，独立维护
--   4. 不随「初始化建表脚本 / 重建完整演示环境」主库批量任务执行，需单独触发
-- ============================================================
...
(
    ...
    `date_call_exists`           decimal(10, 4)  DEFAULT NULL COMMENT '赎回行权剩余期限-最新（年）',
    ...
    `date_inright_exists`        decimal(10, 4)  DEFAULT NULL COMMENT '含权债剩余期限-最新（年）',
