...
13. `createPendingSteps`：
    ...
    - 其他审批节点 → `resolveApprovalHandlers` 展开角色/人员后，**排除本批次真正审核过（submit/approve/reject）的 `handler_id`**（发起人 submit 计入；pending、skipped、auto_process 不计入），再按剩余处理人插入 `pending` 步骤；配置本非空但剔光时抛「下一节点无可用审批人（已排除本流程已参与人员）」。
