...
> 角色定位：对 **ABS 债**（`rrs_securityinfo.abs_flag=1`）发起禁止库（15/16/17/23）入/出池申请。主体级禁投同步 **排除** ABS，故 ABS 必须经本入口单独调整。
...
| 链路 | 对象 | 池范围 | 说明 |
|------|------|--------|------|
| 禁投池调整 · 主体 | 发行主体 | 15/16/17/23 | 通过后同步旗下**非 ABS** 债券 |
| **禁投池调整 · ABS债** | 单只 ABS 债 | **15/16/17/23** | 本期新增；债级，不同步主体下其他债 |
...
- **ABS 详情**：证券信息只读 → 当前所在池 → 可调入/可调出（仅 15/16/17/23）→ 校验 → 流程选择 → multipart 提交。
...
| `queryAdjustPoolList` | 可调池，**硬编码仅 15/16/17/23** |
...
2. **目标池**：手工项 `targetPoolId ∈ {15,16,17,23}`，否则「ABS禁投池调整手工目标池仅允许 15、16、17、23」。  
