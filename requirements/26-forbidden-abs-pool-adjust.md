...
- **ABS 详情**：证券基本信息的字段及顺序与证券池调库完全一致（步骤1可编辑、步骤2只读）。页面查询 `115004000=担保人`、`115203000=差额支付承诺人`、`115202000=债务主体`、`115201000=原始权益人` 四类有最新内评的关系主体，按映射排序号 `1/2/3/4` 升序、同类型内评时间倒序，并默认选中第一条。ABS 显示“权益人、自选权益人、担保人主体内评分”，自选权益人优先并由后端重查防伪；两者均未选时阻断。后续为当前所在池 → 可调入/可调出（仅 15/16/17/23）→ 校验 → 流程选择 → multipart 提交；固定底栏始终为内容区预留安全间距。
