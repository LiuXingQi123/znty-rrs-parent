...
| 类型 | 规则方法 | 失败原因 |
|---|---|---|
| 债券 bond | `inCheckBondMaturity` / `inCheckMainGradeRule` | 失败文案为检查项表述（可多条并存）：债券已到期；主体债入库矩阵未配置允许池；目标池不在矩阵/特殊债天花板范围内；**未配置主体内评分档**（正式证券无内评禁止入信用债/境外债 1～5 级；临时代码默认档 `4`）。矩阵：内评档×剩余期限（含权回售按行权期限、赎回按 `date_exists`，÷365）。**担保债取孰高**；可转债/`releaseRules` 跳过。观察名单不跳过，按标准最好档封顶。私募/ABS 非 1 档、永续（含 1 档）、次级非 1 档在标准最好档上至少下调一级。重点观察禁止新增分级库（强担保豁免），已在 1～4 级只能去五级或出库。境外债按 `inner_sort` 套同一天花板。 |
...
| 债券大库/主体评级矩阵 | 私募/永续/次级/ABS 在标准最好档上至少下调一级、担保孰高、含权期限、观察名单封顶、重点观察禁新增、境外债同档 | 已落地（选池+校验+证券池审核页改判）；观察识别含主体级 issuer_code；白名单流程当前空集选不中 | 老系统观察是跳过矩阵，新需求改为封顶。已在库不符由定时任务生成待办，不自动出池。CRMW 按需求豁免 |
