...
- 后端 SQL：与列表同口径，`SELECT DISTINCT si.security_type, dst.security_type_name FROM rrs_securityinfo si LEFT JOIN dict_security_type dst ... WHERE si.security_type NOT IN ('crmw','company') AND si.security_status != 'D' ORDER BY dst.sort_order ASC, si.security_type ASC`（同上，仅排除退市 `D`；`U` 未知可出现在下拉候选来源证券中）
...
| 起息日期 | `carryDate` | 居中 |
| 到期日 | `maturityDate` | 居中 |
| 剩余期限 | `dateExists` | 库字段 `date_exists`（DECIMAL(10,4)，**天**）；列表表格前端 ÷365 展示为年（四位小数）；证券基本信息区保留天数输入作为原始值，旁侧以深色主数值突出展示「折算年限 x.xxxx年」（`formatRemainTermYears`）。含权债剩余期限 `dateInrightExists`、赎回行权剩余期限 `dateCallExists`、回购剩余期限 `dateRepurchaseExists` 单位是**年**，页面按年录入、不再 ÷365。矩阵：普通债 `date_exists`÷365；含权回售用年字段、赎回用 `date_exists`÷365；`date_call_exists` 仅展示/落库、不参与匹配。**为空时默认最长档（>5 / `GT_5`）继续走矩阵，不跳过** |
...
非 ABS 和 ABS 都使用上述四类关系主体：页面分别展示为“担保人”和“权益人”。页面首次加载时按接口顺序默认选中第一条，用户可改选；用户在页面清空选择时，前端阻止进入下一步。后端对接口直调或遗漏参数同样直接取关联评级主体接口返回的第 1 条（不按关系类型二次筛选）：ABS 写入权益人，非 ABS 写入担保人；无关联主体时 ABS 仍返回“请选择权益人或自选权益人”。ABS 还可从全市场主体选择自选权益人，自选存在时优先于普通权益人，清除后回退普通权益人。任一选择变化均重新查询可调入池并清空原有调入选择，不缓存查询结果、不重新查询最近信评报告。
