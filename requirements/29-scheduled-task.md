...
| `bond_grade_inconformity_alert` | 不符合主体债入库规则提醒 | `0 0 9 * * ?` | 无需参数。扫描已在 1～5 级但按当前特殊债规则不再允许的债券，写入 `ip_grade_rule_alert` 待办；人工在「我的事宜」分级规则提醒页签处理，**不自动出池**。对齐老系统 InconformityMaingrade2Job |
