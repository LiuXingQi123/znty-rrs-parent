...
| `company_outer_rating_not_aa_minus_auto_out` | 外评非AA-及以下主体自动出池 | `0 10 23 * * ?` | 每天 23:10 执行。`poolIds` 可选，默认 `[17]`（17（黑名单质押库）），与关系配置绑定池取并集；`limitPoolIds` Demo 为 `[]`（省略或 `[]` 均不追加额外拦截）。已在目标池且不满足（一）禁止库 15 /（二）近一年孰低 AA-及以下 /（三）重点观察 23 时出主体；近一年无认可外评不出。仅认机构 2/3/4/5/6/7/13/14/19/20。成功后顺带出同池旗下债 |
...
| `company_same_pool_bond_auto_in` | 主体下债券自动入库 | `0 20 23 * * ?` | 每天 23:20 执行。`poolIds` 可选，默认 `[15]`（15（债券禁止库）），与关系配置绑定池取并集；主体已在该池时，旗下未到期（含当天）且未在同一池的债券自动入池；入池后按互斥/反向限制关系自动调出原池并记日志 |
...
2. 出库条件：已在目标池，且 **（一）（二）（三）均不满足**——不在禁止库 15、不在重点观察 23、近一年认可外评存在且孰低不属于 AA-及以下。
3. SQL 只查条款（二）反面（已在目标池且孰低不属于 AA-及以下）；仍在 15/23 的主体由 Java 按在池名单排除。近一年无认可外评 **不出**。
...
6. 主体出池成功后，顺带调出同池旗下 `bond` 大类；调整原因如 `外评非AA-及以下主体自动出池（近一年孰低外评：AAA）`，同池债追加 `（同池旗下债）`。
7. 调出限制池（关系 12 / `out_restrict`）对主体与旗下债分别拦截。
