...
4. 数据源同 7.3：近一年（日历年）10 家认可机构多评级取孰低；一年以前忽略。仅认 `2/3/4/5/6/7/13/14/19/20`。
...
4. （二）数据源 `ais_inv_ods.wind_cbondissuerrating`：近一年内认可机构外评取 **孰低**；一年以前忽略。命中 `AA-` / `A±` / `BBB…` / `BB…` / `B…` / `CCC/CC/C` / `D`（**不含** `AA`/`AA+`/`AAA`）。仅认 `b_info_creditratingagency IN ('2','3','4','5','6','7','13','14','19','20')`。无认可外评不因（二）入库。
