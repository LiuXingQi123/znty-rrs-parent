...
- **主体区**左右两栏：
  - **左侧侧边栏**：标题「投资池树」+「新增顶级」按钮 + 刷新按钮 + 树搜索框 + `el-tree`（`node-key="id"`，默认全部收起，自定义节点模板：父节点 folder 图标随展开状态切换、叶子 document 图标，hover 显示「+添加子节点」「删除节点」）。关系/规则选池弹窗仍 `default-expand-all`。
  - **右侧内容区**：`pool-header`（池名 + 编码 + 启用徽章）+ `el-tabs` 三个标签页：
    - **基础配置**：基础信息表单（名称/`poolCode` 可创建下拉/恒生池名/投资市场复选/投资品种复选/最大上限/锁池/调入冻结期/外部排序/内部排序/调入研报限制/调出研报限制/描述）+ 审批流程表单（6 个流程下拉）+ 刷新/保存按钮。
...
| 风险 | `blacklist` | 黑名单 | 黑名单质押库等 | 禁投查询 |
| 风险 | `restricted` | 限制名单 | 重点观察名单 | 禁投查询，无全局禁止 |
...
- 请求体：`{parentId, poolName, poolCode, innerSort?, templatePoolId?, inheritParentConfig?, operatorId?}`
- 后端 `addChildPool`：校验 parentId/poolName/poolCode 非空、父池存在、编码唯一；模板选择（显式传 templatePoolId 优先，否则 inheritParentConfig 用父池当模板）；`buildChildPool`（**`poolType` 强制继承父池**不读 req，`poolLevel=父.level+1`，`outerSort=父.outerSort`，`innerSort` 未传取父下最大+1，**`poolCode` 取请求值**）；插入+事件+模板复制。
- 前端打开弹窗时默认把 `templatePoolId` 设为父节点 id（「以父节点为模板」）；编码同样为可创建下拉。
...
`addSeedPoolList` 幂等：`queryPoolTotalCount > 0` 直接返回现有列表。首次初始化仅创建信用债大库+一~五级库、境外债库、转债库、CRMW 根池。完整树（含公司股票库、转债产品库、股票产品库 A/H 股、公司港股库、公司基金库、基金产品库、专户债券产品库）以 `sql/rrs_pool_init_demo_data.sql` 为准；转债产品库 `outer_sort=7`、专户债券产品库 `outer_sort=8`，依次紧挨信用债大库(new)（`outer_sort=9`）之上。股票/股票产品树绑 `stock:*` 一般/快速/批量入出库流程，基金树绑 `fund:*` 对应流程，与债券流程分开。该接口前端未调用，供首次部署/测试手动调用。
