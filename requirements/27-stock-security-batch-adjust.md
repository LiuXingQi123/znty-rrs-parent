...
| 可选证券筛选 | 代码 / 简称 / 市场 / 是否特征 | 另加 **来源池（多选，必选≥1）**、**发行主体**；是否特征含 ABS/担保/永续/次级/私募/含权，多选 AND |
| 来源池 | 无 | 固定白名单 6 项，见 §2 |
...
- 菜单：债券研究 → 存量证券批量调整  
- 可选证券区：来源池多选（必选）、发行主体、代码、简称、市场、是否特征，默认 10 条/页；列表「剩余期限(年)」同 [12]（`dateExists` 库内为**天**，前端 ÷365 展示），并展示 ABS/担保/永续/次级/私募/含权六列「是/否」Tag
