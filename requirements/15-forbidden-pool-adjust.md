...
主体调库写入时 `pool_type` 为 `forbidden`/`observe`/`blacklist`/`restricted`；`security_code` 存主体代码；旗下债券自动调整记录 `security_code` 存债券 `wind_code`，`adjust_type='自动调整'`，`adjust_reason='主体"XXX"调入/调出，旗下债券自动同步调整'`。`adjust_batch_no` 以 `BOND` 前缀。
