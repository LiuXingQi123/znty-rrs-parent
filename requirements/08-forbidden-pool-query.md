...
**本页查询三类主体风险池**：禁投池（15 / `forbidden`）、观察池（16 / `observe`）和黑名单质押库（17 / `blacklist`）。复用 `ip_pool_status`（投资池当前状态表），通过 `pool_type IN ('forbidden','observe','blacklist')`、`ips.is_deleted=0`、`ips.audit_status='20'` 查询当前已生效数据。主体级状态按 `ips.security_code=ais_inv_ods.wind_cbondissuer.s_info_compcode` 关联有效 Wind 主体（**该表为债券+主体粒度，JOIN 前须按 `s_info_compcode` 去重**）；债券仍按 Wind 证券代码关联 `rrs_securityinfo`。
...
SELECT ips.id, ips.security_code, ips.security_short_name, ips.adjuster_name,
       ...
       COALESCE(wci.s_info_compname, bi.issuer, ips.security_short_name) AS issuer,
       bi.maturity_date, bi.delist_date, bi.repurchase_date
...
) wci ON wci.s_info_compcode = ips.security_code AND ips.security_type = 'company'
<where>
    ips.is_deleted = 0 AND ips.audit_status = '20'
    ...
    <if issuer> AND COALESCE(wci.s_info_compname, bi.issuer, ips.security_short_name) LIKE CONCAT('%', #{issuer}, '%') </if>
</where>
