...
- `sirm_securityinfo`（证券信息表）：关联键 `wind_code = ips.security_code`，提供 `issuer/maturity_date/delist_date/repurchase_date`。
...
LEFT JOIN sirm_securityinfo bi ON bi.wind_code = ips.security_code
...
- SQL：`sql/sirm_security_pool_adjust_schema.sql`（ip_pool_status）、`sql/sirm_pool_init_schema.sql`
