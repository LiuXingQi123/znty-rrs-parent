...
- **detail 页**：返回按钮 + 证券基本信息（字段与顺序同 [04]/[11]；非 ABS 展示担保人，ABS 展示权益人和自选权益人，关系主体默认选中第一条，自选优先联动“担保人主体内评分”；`115202000` 展示为“债务主体”；「剩余期限(天)」`date_exists` 隐藏保留，只读「证券期限」展示 `date_exists_str`；含权/赎回行权/回购剩余期限 **年**）+ 发行主体最近三年及一期财务数据（只读，可切换城投债/产业债指标口径）+ CRMW 基本信息（只读）+ 当前所在池 + 调库操作卡（步骤 1 选池 / 步骤 2 校验确认）+ 流程选择弹窗 + 信评报告选择弹窗。
