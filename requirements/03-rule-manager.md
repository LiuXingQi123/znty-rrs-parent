...
统一 `POST`、`@RequestBody`、返回 `ApiResponse<T>`（`code=0` 成功）；分页 `PageResult<T>`。
...
建表脚本 `sql/sirm_rule_schema.sql`，演示数据 `sirm_rule_demo_data.sql`。共 10 张表，无数据库物理外键（靠应用层维护）。
...
- 全局异常处理 `ExceptionConfig`：`BizException` → `ApiResponse.fail(code, message)`（默认 400）；其他 Exception → 记日志 + `ApiResponse.fail(500, "系统繁忙")`。
...
- 前端：`znty-sirm-ui/rule_manager.html`（`loadRules`、`loadTestCases`、`saveRule`、`toggleStatus`、`runTest`、`addTestCaseFromDialog`、`runSingleTest`、`runAllTests`、`deleteTestCase`、新增参数 Dialog）
...
- SQL：`sql/sirm_rule_schema.sql`、`sql/sirm_rule_demo_data.sql`
