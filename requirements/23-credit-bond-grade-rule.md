...
>
> **特殊债消费**：一只券多种类型时只留一个标签，后写覆盖先写：私募 → 次级 → 永续 → 担保（例如永续+担保按担保）。口径：ABS/私募/永续/次级 1 档**只能调入一级库**（仅一级）；次级 2+/2/2- **下调一级**（矩阵最好档再降一档且只留那一档）；**至少下调一级**=从该降一档起开到五级（最好 1 则 2～5）。私募/永续/次级 **「1 档」只看发债主体内评**（`innerIssuerRating`）；ABS **「1 档」只看页面所选担保人内评**（`innerGuarantorRating`，无担保人则不是 1 档）。然后：
> 1. ABS（`absFlag=1` 或类型 abs/abn）始终走 ABS，不看覆盖标签：担保人内评 1 档只能调入一级库，无担保人或非 1 档至少下调一级。覆盖后仍是私募或永续：发债主体内评 1 档只能调入一级库，其余至少下调一级。覆盖后仍是次级：发债主体内评 1 档只能调入一级库，2+/2/2- 下调一级（只留那一档），其余至少下调一级。
