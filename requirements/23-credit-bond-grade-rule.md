...
>
> **特殊债消费**：一只券多种类型时只留一个标签，后写覆盖先写：私募 → 次级 → 永续 → 担保（例如永续+担保按担保）。口径：「可调入一级库」=信用债 1～5 都可进（私募/ABS 1 档）；「只能调入一级库」=仅一级（次级 1 档）；「下调一级」=矩阵最好档再降一档且只留那一档（最好已是五级则仍是五级）；「至少下调一级」=从该降一档起开到五级（最好 1 则 2～5）。**「1 档」只看发债主体内评**（`innerIssuerRating`），不用担保人内评孰高。然后：
> 1. ABS（`absFlag=1` 或类型 abs/abn）始终走 ABS，不看覆盖标签：发债主体内评 1 档可调入一级库，其余至少下调一级。覆盖后仍是私募：同上。覆盖后仍是永续：发债主体内评 1 档下调一级，其余至少下调一级。覆盖后仍是次级：发债主体内评 1 档只能调入一级库，2+/2/2- 下调一级，其余至少下调一级。
