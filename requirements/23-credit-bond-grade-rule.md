...
> **期限口径**：调库校验消费矩阵时，期限档按**剩余期限（年）**匹配。单位：`date_exists` 是**天**，`date_inright_exists` / `date_call_exists` / `date_repurchase_exists` 是**年**。普通债把 `date_exists` ÷365；含权债回售直接用 `date_inright_exists`/`date_repurchase_exists`（年），赎回把 `date_exists` ÷365，两者都有取更短。`date_call_exists`（赎回行权剩余期限，年）仅展示/落库，**不参与**矩阵匹配。期限为空时默认最长档（期限>5 / `GT_5`）继续走矩阵，**不跳过**。无启用期限档或年数落不进任何档时报「无法匹配债券期限档」。
>
> **特殊债消费**：一只券多种类型时只留一个标签，后写覆盖先写：私募 → 次级 → 永续 → 担保（例如永续+担保按担保）。口径：ABS/私募/次级 1 档**只能调入一级库**（仅一级）；永续 1 档及次级 2+/2/2- **下调一级**（矩阵最好档再降一档且只留那一档）；**至少下调一级**=从该降一档起开到五级（最好 1 则 2～5）。私募/永续/次级 **「1 档」只看发债主体内评**（`innerIssuerRating`）；ABS **「1 档」只看页面所选担保人内评**。所选 `guarantorCode` 按 `ais_inv_analysis.v_inv_grade_result.windcode` 查询最新 `total_score`，不按位置读取 `rrs_securityinfo.inner_guarantor_rating`；无担保人或新表无评分记录时不是 1 档。然后：
> 1. ABS（`absFlag=1` 或类型 `abs`；`abs_all` 不算）始终走 ABS，不看覆盖标签：担保人内评 1 档只能调入一级库，无担保人或非 1 档至少下调一级。覆盖后仍是私募：发债主体内评 1 档只能调入一级库，其余至少下调一级。覆盖后仍是永续：发债主体内评 1 档下调一级（只留那一档），其余至少下调一级。覆盖后仍是次级：发债主体内评 1 档只能调入一级库，2+/2/2- 下调一级（只留那一档），其余至少下调一级。
> 2. 标签是担保债，或券/主体已在观察池：入库不得高于矩阵最好档，从该档开到五级。担保查矩阵前先把主体内评和担保人内评取更好的一档（只影响查哪一格，不把发债主体非 1 档当成 1 档）。
> 3. 普通债：只进矩阵单元格里的池。
> 可转债/可交换债/可分离转债/CRMW 不能进信用债 1～5。境外债不走本矩阵。
>
> **观察池**：券自己在观察池，或主体级记录（`ip_pool_status.security_code = issuer_code`）在观察池，按上面第 2 条处理（不得高于矩阵最好档，开到五级），不跳过矩阵。若该券同时是 ABS/私募/次级/永续，仍走第 1 条特殊债，观察池不改结果。
>
> **重点观察名单**：券或主体在 `pool_type=restricted` 时走 `checkRestricted`，不走类型分支。未在信用债 1～5 不得新增；已在 1～4 级只能去五级或出库；已在五级不可上调。有担保且担保人内评 1 档可豁免。CRMW 事先排除分级库。含权债不改档位，只改期限。
>
