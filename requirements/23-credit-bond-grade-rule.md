...
> **期限口径**：调库校验消费矩阵时，期限档按**剩余期限（年）**匹配。普通债及含权债赎回侧解析 `date_exists_str`（支持“3年6个月3天”“3年6月3日”“6个月3天”“6月3日”“6天/6日”等格式；年原值保留、月÷12、日÷365），不再将 `date_exists` 总天数直接÷365。含权债回售侧直接用 `date_inright_exists` / `date_repurchase_exists`（年），两侧都有时取更短。`date_call_exists`（赎回行权剩余期限，年）仅展示/落库，**不参与**矩阵匹配。期限文本为空或格式不正确时默认最长档（期限>5 / `GT_5`）继续走矩阵，**不跳过**。无启用期限档或年数落不进任何档时报「无法匹配债券期限档」。
>
