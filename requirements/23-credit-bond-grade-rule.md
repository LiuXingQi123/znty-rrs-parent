...
> 1. ABS（`absFlag=1` 或类型 abs/abn）始终走 ABS，不看覆盖标签：担保人内评 1 档只能调入一级库，无担保人或非 1 档至少下调一级。覆盖后仍是私募：发债主体内评 1 档只能调入一级库，其余至少下调一级。覆盖后仍是永续：发债主体内评 1 档下调一级（只留那一档），其余至少下调一级。覆盖后仍是次级：发债主体内评 1 档只能调入一级库，2+/2/2- 下调一级（只留那一档），其余至少下调一级。
