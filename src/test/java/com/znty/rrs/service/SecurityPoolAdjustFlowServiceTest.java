...
public class SecurityPoolAdjustFlowServiceTest {
    ...
    public void submitAdjustAuditShouldUseAdminOwnPendingStepWhenAdminIsHandler() {
        ...
        // 构建待处理流程步骤测试数据
        IpAdjustStepBo adminStep = buildPendingStep(11L, "10000", "预留管理员");
        // 构建审批请求测试数据
        SecurityPoolAdjustAuditReq req = buildReq(10L, "10000", "预留管理员", "同意");
        when(mapper.queryAdjustStepById(10L)).thenReturn(otherStep);
        when(mapper.queryPendingStepByHandler(1L, "BATCH001", 10103L, "10000")).thenReturn(adminStep);
    ...
    public void submitAdjustAuditShouldKeepAdminSuffixWhenAdminActsForOtherHandler() {
        ...
        SecurityPoolAdjustAuditReq req = buildReq(10L, "10000", "预留管理员", "同意");
        when(mapper.queryAdjustStepById(10L)).thenReturn(otherStep);
        when(mapper.queryPendingStepByHandler(1L, "BATCH001", 10103L, "10000")).thenReturn(null);
